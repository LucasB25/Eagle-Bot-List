...
exports.embedSplitter = async function embedSplitter(name, valueArray, embed) {
  ...
  if (bigString.length < 1024)
    return embed.addFields({ name: name, value: bigString });
  ...
  await substrings.forEach((substring) => {
    if (substring.length) embed.addFields({ name: name, value: substring });
  });
...
exports.timezoneWithDate = (prop, guild) => {
  const { capitalize } = require("./util");
  return `${capitalize(
    moment.tz(prop, guild.settings.get("timezone")).fromNow()
  )} (${moment
    .tz(prop, guild.settings.get("timezone"))
    .format("MMMM Do, YYYY")})`;
};
